...
//		if(outRootDir == null) throw new IllegalStateException("'outRootDir' property must be not null.");
	...
	private class TemplateProcessor {
		...
		private void generateNewFileOrInsertIntoFile( String templateFile,String outputFilepath, Map templateModel) throws Exception {
			...
			File absoluteOutputFilePath = null;
			//if(outputFilepath.split(File.separator)[0].equals("jsp")){
			//	absoluteOutputFilePath = FileHelper.mkdir(templateModel.get("outRootJsp").toString(),outputFilepath.replaceFirst("jsp", System.getProperty("modulName")));
			//}else{
				absoluteOutputFilePath = FileHelper.mkdir(templateModel.get("outRootFunction")+System.getProperty("modulName"),outputFilepath);
			//}
