...
public class CommandLine {
	...
	private static void processLine(Scanner sc) throws Exception {
		...
		if("gen".equals(cmd)) {
			...
			if(args.length < 2){
				System.out.println("请输入正确的参数，格式：gen 数据库表名称 主模块名  数据库表映射的类名（可选）");
				return;
			}
			if(args.length == 3){
				System.setProperty("className", args[2]);
			}
			System.setProperty("modulName", args[1]);
//			facade.g.setIncludes(getIncludes(args,1));
			facade.generateByTable(args[0],getTemplateRootDir());
//			if(SystemHelper.isWindowsOS) {
//			    Runtime.getRuntime().exec("cmd.exe /c start "+GeneratorProperties.getRequiredProperty("outRootFunction").replace('/', '\\'));
//			    Runtime.getRuntime().exec("cmd.exe /c start "+GeneratorProperties.getRequiredProperty("outRootJsp").replace('/', '\\'));
//			}
